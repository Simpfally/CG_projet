...
machine = SVM()
machine = torch.nn.Linear(3781,1)
optimizer = optim.SGD(machine.parameters(), lr=0.1)  # Our optimizer
machine.train()
max_epochs = 100
for epoch in range(1, max_epochs+1):
    ...
    for i in range(0, n):
        ...
        loss = args.c * torch.mean(torch.clamp(1 - output * y, min=0))  # hinge loss
        loss += (1-args.c)*(machine.weight**2).sum() / 2.0
